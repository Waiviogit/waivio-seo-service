...
const getDirectoryByObjectType = (objectType, social) => {
  if (!social) return 'object';
  const types = {
    list: 'checklist',
    page: 'checklist',
    default: 'object',
  };
  return types[objectType] || types.default;
};

const formObjectLinks = ({ objects, host, social }) => {
  const links = [];
  for (const object of objects) {
    const directory = getDirectoryByObjectType(object.object_type, social);
